...
const Home: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if(valueSearch) {
      lstResult = lstResult.map(data => {
        return {
          ...data,
          listPackage: data.listPackage.filter(d => d.name.toLocaleLowerCase().indexOf(valueSearch.toLocaleLowerCase()) > -1) || []
        }
      })
    }
  ...
  return (
      <div className='container-main home-wrapper'>
        <h5>Xin chào, Ánh Trương!</h5>
        <IonSearchbar placeholder='Tìm gói' onIonInput={(e) => handleSearch(e)} class='searchbar' />
        <div className='category__list'>
          {lstTag.map((tag) => (
            <TagCategory item={tag} key={tag.key} handleChange={handleClickTag} selectedItem={selectedItem} />
          ))}
        </div>
        {lstDataResult.map((data) => (
          data.listPackage.length > 0 && <PackageCard lstData={data.listPackage} title={data.tageName} className='mt-25' key={data.key} />
        ))}
